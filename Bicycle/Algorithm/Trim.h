...
//---------------------------------------------------------------------------
	...
	inline ForwardIteratorT
		 trim_end_iter_select(ForwardIteratorT inBegin,	ForwardIteratorT inEnd,
													PredicateT isSpace, std::forward_iterator_tag)
	{
...
//---------------------------------------------------------------------------
	...
	{
    return SequenceT(trim_begin(Bicycle::begin(input),Bicycle::end(input),isSpace),Bicycle::end(input));
	}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
	...
	inline SequenceT trim_right_copy_if(const SequenceT& input,
			PredicateT isSpace)
	{
		return SequenceT(begin(input),
				 trim_end(begin(input), end(input),isSpace));
	}
//---------------------------------------------------------------------------
	template<typename SequenceT>
...
//---------------------------------------------------------------------------
		...
		trim_copy_if(OutputIteratorT output,const RangeT& input, PredicateT isSpace)
	{
      ...
      trimEnd=	trim_end(Bicycle::begin(lit_range),Bicycle::end(lit_range), isSpace);

    std::copy(trim_begin(Bicycle::begin(lit_range), trimEnd, isSpace),trimEnd, output);
